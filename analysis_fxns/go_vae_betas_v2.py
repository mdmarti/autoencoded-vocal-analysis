...
old_spec_dir = [os.path.join(root,animal,'old_mod_spec_centroids_v3',day) for animal in trainAnimals for day in trainDays]
...
old_data = make_old_spec_file(km_old_points, train_audio_dirs[0], train_segment_dirs[0], \
    old_spec_dir[0], preprocess_params)
...
for ind, beta in enumerate(beta_list):
	...
	save_dir=os.path.join(root,'model_288_2_beta_' + str(beta))
